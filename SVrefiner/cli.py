...
@click.group()
@click.version_option("0.2", prog_name="SVrefiner", message="""
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
SVrefiner - A Python tool for generating refined Structured Variants (rSVs)  
Version 0.2 - Contact: fenglostmet@tju.edu.cn
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
""")
def cli():
...
@cli.command("align")
@common_options
def align_time(vcf: str, ref: str, out: str, threads: int, write_matrix: bool):
...
@common_options
def process_vcf(vcf: str, ref: str, out: str, threads: int, write_matrix: bool):
...
@cli.command("run-all")
@common_options
def run_all(vcf: str, ref: str, out: str, threads: int, write_matrix: bool):
    ...
    try:
        ...
        if writedown:
            ...
            rSV_count, sample_names2, gt_buffer = load_encoded_gt_from_matrix_dir(out)
            logging.info(f"rSV count: {rSV_count:,}")
            vcf_generate_from_gt(rSV_meta_csv, rSV_vcf, sample_names2, gt_buffer)
        else:
            click.echo("Generating rSV.vcf in streamed mode (no X/T)...")
            rSV_count, sample_names = build_rsv_vcf_streamed_nobuf(
                vcf_dir=out,
                output_dir=matrix_dir,
                meta_csv_file=rSV_meta_csv,
                output_vcf_file=rSV_vcf,
...
@common_options
def make_rSV(vcf: str, ref, out: str, threads: int, write_matrix: bool):
